package readWrite;

import java.util.Set;

import graph.Point;
import graph.Vertex;

public class CoordinateConversion {

    public Point referencePoint;
    private final double METERS_IN_DEGREE = 111321.377778;


    public CoordinateConversion() {
        referencePoint = new Point(30.32268115454809, 59.93893094417527);
    }

    public CoordinateConversion(Point refPoint) {
        referencePoint = refPoint;
    }


    public <T extends Point> CoordinateConversion(Set<T> vertexSet) {
        referencePoint = findCenter(vertexSet);
    }   


    private <T extends Point> Point findCenter(Set<T> vertexSet) {
        Point ans = new Point(0, 0);
        double minX = Double.MAX_VALUE, maxX = Double.MIN_VALUE, minY = Double.MAX_VALUE, maxY = Double.MIN_VALUE;
        for (T ver : vertexSet) {
            minX = Math.min(minX, ver.x);
            maxX = Math.max(maxX, ver.x);
            minY = Math.min(minY, ver.y);
            maxY = Math.max(maxY, ver.y);
        }
        ans.x = (minX + maxX) / 2.0;
        ans.y = (minY + maxY) / 2.0;
        return ans;
    }
        
        
    public <T extends Point> void toEuclidean(T p) {
        double lat = p.x;
        double lon = p.y;
        p.x = METERS_IN_DEGREE * Math.cos(referencePoint.y * Math.PI / 180) * (lat - referencePoint.x);
        p.y = METERS_IN_DEGREE * (lon - referencePoint.y);
    }


    public <T extends Point> T fromEuclidean(T p) {
        T ver = (T) p.copy();
        double x = p.x;
        double y = p.y;
        ver.y = y / METERS_IN_DEGREE + referencePoint.y;
        ver.x = x / METERS_IN_DEGREE /  Math.cos(referencePoint.y * Math.PI / 180) + referencePoint.x;
        return ver;
    }
}
